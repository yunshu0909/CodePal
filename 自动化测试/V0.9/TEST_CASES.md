...
- 断言：
  ...
  - 参数包含 `overwrite=false`、模板选择结果、Git 模式
  - 显示成功弹窗摘要（项目路径/目录数/配置状态）

### TC-FE-06（P0）execute 失败展示失败弹窗并可关闭
- 类型：Frontend Integration
- 覆盖：失败分支
- 断言：
  - 显示失败弹窗（错误标题/原因/提示）
...
### TC-FE-07（P0）execute 失败后可重试并转成功
- 类型：Frontend Integration
- 覆盖：失败重试分支
- 断言：
  - 首次执行失败弹出失败弹窗
