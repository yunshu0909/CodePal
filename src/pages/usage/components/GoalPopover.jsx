...
export default function GoalPopover({ goal, month, onSave, onClose }) {
  const [text, setText] = useState(() =>
      goal ? `${goal.unit === 'K' ? goal.value / 1000 : goal.value}${goal.unit === 'K' ? 'M' : goal.unit}` : ''
    ),
  ...
  const commit = async () => {
    ...
    if (!next) {
      setError('请输入大于0的M或B数值')
      return
    }
    if (goal?.value === next.value && goal?.unit === next.unit) {
      closed.current = true
      onClose()
      return
    }
    ...
    setSaving(true)
    try {
      await onSave(next.value, next.unit)
      closed.current = true
      onClose()
    } catch {
      setError('保存失败，请重试')
  ...
  const parsed = parseGoalInput(text),
    tokens = parsed ? parsed.value * (parsed.unit === 'B' ? 1e9 : 1e6) : 0
  return (
    <div className="uc-popover np-pop" ref={element} role="dialog" aria-label="用量目标">
      <b>用量目标</b>
      <label>
        每天
        <input
      ...
      <small>
        每月目标 = 每天 × 当月天数（{getMonthDays(month)}天
        {tokens ? ` = ${formatToken(tokens * getMonthDays(month))}` : ''}）· 回车保存
      </small>
    </div>
  )
}
