...
export default function DayRing({total,target,size=36,showPercent=false}) {
  ...
  const color=tier==='over'?'var(--purple)':tier==='done'?'var(--green)':'var(--blue)'
  return <span className="uc-ring" data-testid="day-ring" style={{width:size,height:size}}>
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} data-tier={tier} aria-label={`${percent}%`}>
      <circle cx={size/2} cy={size/2} r={radius} stroke="var(--track)"/>
      ...
      {tier==='over'&&<circle data-second-circle cx={size/2} cy={size/2} r={radius} stroke="var(--purple-fg)" strokeDasharray={circumference} strokeDashoffset={circumference*(1-Math.min(ratio-1,1))}/>}
    </svg>
