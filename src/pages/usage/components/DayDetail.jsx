...
export default function DayDetail({date,day,today,target}) {
  ...
  return <section className="uc-card np-card uc-detail" data-testid="day-detail" aria-label="选中日模型构成">
    {target>0&&day.total>0&&<DayRing total={day.total} target={target} size={64} showPercent/>}
    <div className="uc-detail-content">
      <div className="uc-detail-heading"><b>{date.slice(5)}{date===today?' · 今天':''}</b><strong>{formatToken(day.total)}</strong>{target>0&&day.total>0&&<><span>目标 {formatToken(target)}</span><span className={`uc-status np-tag np-tag--${{under:'blue',done:'green',over:'purple'}[status.tier]} uc-${status.tier}`}>{status.label}</span></>}</div>
      {rows.map((row,i)=><div className={`uc-model-row np-meter ${row.other?'uc-other':''}`} key={row.name}>
        <span className="uc-model-name" title={row.name}>{row.name}</span>
        <span className="uc-model-bar np-meter-bar"><i style={{width:`${day.total?row.total/day.total*100:0}%`,opacity:[1,.72,.5,.34,.26,.2,.15][i]}}/></span>
