...
export default function PlanCard({
  ...
  return <section className={`plan-card np-card ${view.state === 'paused' ? 'plan-paused' : ''} ${loading && !unset ? 'plan-sk' : ''}`} data-testid="plan-card" data-plan-id={planId}>
  <div className="plan-hd" data-testid="plan-card-header"><b><span className={`plan-app plan-app-${planId}`}><svg viewBox="0 0 16 16" aria-hidden="true">{icon}</svg></span>{name}<span className="plan-type">{metadata?.type || '未知'}</span></b><span>{view.badge && <span className={`plan-tag np-tag ${view.state === 'expired' ? 'plan-dead np-tag--red' : view.warning ? 'plan-warn np-tag--orange' : 'plan-muted np-tag--gray'}`}>{view.badge}</span>}</span></div>
  <div className="plan-top" data-testid="plan-card-number-region"><div className="plan-xw"><span className={`plan-x ${view.bad && !loading ? 'plan-bad' : ''} ${unset || view.total === null ? 'plan-na' : ''}`}>{loading && !unset ? '00×' : view.multiplier}</span>{view.subtitle && <span className={`plan-sub ${unset ? 'plan-link' : view.bad && ['active', 'history'].includes(view.state) && !loading ? 'plan-bad' : ''}`}>{view.subtitle}</span>}</div>
   <table className="plan-kv" data-testid="plan-values"><tbody><tr><td>订阅费</td><td className="plan-price"><Button variant="ghost" className={`plan-price-button ${unset ? 'plan-link' : ''}`} aria-label={`${name} 订阅设置`} onClick={() => setSettings(old => !old)}>{unset ? '设置' : loading ? '$00' : formatUSD(cycle.price)}</Button>{view.estimated && !loading && <span className="plan-est">推算</span>}{settings && (history ? <CyclePricePopover cycle={cycle} onSave={price => onSaveCyclePrice(cycle.id, price)} onClose={close} /> : <PlanSettingsPopover planId={planId} name={name} plan={plan} metadata={metadata} today={today} onSave={onSave} onClose={close} />)}</td></tr><tr><td>等价 API</td><td><span>{loading && !unset ? '$0,000' : formatUSD(view.total)}</span></td></tr>
  ...
  {!unset && (loading ? <div className="plan-models" data-testid="plan-model-list">{[.7, .4, .1].map((f, i) => <div className="plan-mrow np-meter" key={i}><span className="plan-nm">模型名</span><span className="plan-model-bar np-meter-bar"><i style={{
          ...
          }} /></span><span className="plan-v" data-testid={i === rows.length - 1 ? 'plan-model-list-last-row' : undefined}>{!row.other && row.key && (row.cost === null || row.own) ? <><Button variant="ghost" className={`plan-price-button ${row.cost === null ? 'plan-link' : ''}`} aria-label={`${row.name} 价格`} onClick={() => setPricing(old => old === row.key ? null : row.key)}>{formatUSD(row.cost)}</Button>{pricing === row.key && <ModelPricePopover model={row} onRefresh={onRefreshPrice} onSaveLocal={onSaveLocalPrice} onClearLocal={onClearLocalPrice} onClose={closePricing} />}</> : formatUSD(row.cost)}</span></div>)}</div> : view.total === 0 ? <div className="plan-empty">{usage?.missingSource ? `没有找到 ${name} 的本地记录` : '本周期还没有用量'}</div> : null)}
 </section>;
}
