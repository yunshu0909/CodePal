...
export default function PlanCard({
  ...
  {!unset && (loading ? <div className="plan-models" data-testid="plan-model-list">{[.7, .4, .1].map((f, i) => <div className="plan-mrow" key={i}><span className="plan-nm">模型名</span><span className="plan-model-bar"><i style={{
            width: `${f * 100}%`
          }} /></span><span className="plan-v">$000</span></div>)}</div> : rows.length ? <div className="plan-models" data-testid="plan-model-list">{rows.map((row, i) => <div className={`plan-mrow plan-model-${row.other ? 'other' : i}`} data-testid="plan-model-row" key={row.name}><span className="plan-nm" title={row.name}>{row.name}{row.own && <span className="plan-own">自填</span>}</span><span className="plan-model-bar"><i style={{
