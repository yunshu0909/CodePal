 ...
 *
 * 负责：
 * - ~/.codex/config.toml：经 Codex 配置负责人（同一把锁、保格式、先备份）删掉 [mcp_servers.provider_registry] 及其子表
 ...
 */
async function cleanupCodex(homeDir) {
...
/**
 * 读 ~/.claude.json 当前状态（内容 + inode）
 * @param {string} filePath
 * @returns {Promise<{exists: boolean, text: string, ino: number|null, mode: number}>}
 */
async function snapshotJson(filePath) {
  try {
    const stat = await fs.stat(filePath)
    return { exists: true, text: await fs.readFile(filePath, 'utf8'), ino: stat.ino, mode: stat.mode & 0o777 }
  } catch (error) {
    if (error.code === 'ENOENT') return { exists: false, text: '', ino: null, mode: 0o600 }
    throw error
  }
}
 ...
 */
async function cleanupClaudeJsonOnce(filePath, deps) {
  const before = await snapshotJson(filePath)
  ...
  const backupTemp = await writeTemp(`${filePath}.codepal.bak`, before.text, before.mode)
  let newTemp = null
  try {
    newTemp = await writeTemp(filePath, next, before.mode)
    if (deps.beforeClaudeJsonCommit) await deps.beforeClaudeJsonCommit(filePath)
    const now = await snapshotJson(filePath)
    if (!now.exists || now.text !== before.text || now.ino !== before.ino) throw codedError('CONFLICT')
    await fs.rename(newTemp, filePath)
    newTemp = null
    await fs.rename(backupTemp, `${filePath}.codepal.bak`).catch(() => fs.rm(backupTemp, { force: true }))
    return 'removed'
  } catch (error) {
 ...
 */
async function cleanupClaudeJson(homeDir, deps = {}) {
  const filePath = path.join(homeDir, '.claude.json')
  const attempts = deps.maxAttempts || 3
  for (let i = 0; i < attempts; i++) {
    try {
      return await cleanupClaudeJsonOnce(filePath, deps)
    } catch (error) {
...
async function runLegacyProviderRegistryCleanup({ homeDir, store }, deps = {}) {
  ...
  const codex = await cleanupCodex(homeDir)
  const claude = await cleanupClaudeJson(homeDir, deps)
