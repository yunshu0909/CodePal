...
const SCHEMA = 1,
  SEMANTICS = `source-day-v1:daily-${DAILY_SUMMARY_SCHEMA_VERSION}`,
  DAY = 86400000,
  EARLIEST_TTL = 3600000
...
const validDay = (key) =>
  ...
  Number.isFinite(midnight(key).getTime()) &&
  dayKey(midnight(key)) === key
const nextDay = (key) => dayKey(new Date(midnight(key).getTime() + DAY))
const sourcePath = (home, id) =>
  path.join(
    home,
    '.' + (id === 'claude' ? 'claude' : id === 'codex' ? 'codex' : 'dsh'),
    id === 'claude' ? 'projects' : 'sessions'
  )
...
function createStatisticsStorage(home = os.homedir()) {
  ...
  const filename = (key) => {
    if (key !== 'metadata' && !validDay(key)) throw Error('INVALID_DAY')
...
function createSharedUsageStatistics({
  homeDir = os.homedir(),
  storage = createStatisticsStorage(homeDir),
  scanFn,
  sourceStatusFn,
  earliestFn,
  sourceEarliestFn,
  ...
  const status =
    ...
    (async (id) => {
      try {
        const s = await fs.stat(sourcePath(homeDir, id))
        if (!s.isDirectory()) throw Error('SOURCE_NOT_DIRECTORY')
        return 'present'
      } catch (e) {
    ...
    (async (key) => {
      try {
        return normalizeDailySummary(
          JSON.parse(await fs.readFile(path.join(homeDir, '.ai-workbench', 'daily-stats', key + '.json'), 'utf8')),
          key
        )
      } catch (e) {
        if (e.code === 'ENOENT' || e instanceof SyntaxError) return null
  ...
  const logEarliest =
    ...
    ((id) =>
      id === 'claude'
        ? findEarliestClaudeDate(sourcePath(homeDir, 'claude'))
        : findEarliestCodexDate(sourcePath(homeDir, 'codex')))
  ...
  async function getSourceEarliestDate(id) {
    ...
    let fromLogs = null
    try {
      const value = await logEarliest(id)
      fromLogs = validDay(value) ? value : null
    } catch {
      /* Unreadable logs only lose the log-side hint. */
    }
    let fromCache = null
    for (const key of (await storage.list()).sort()) {
      const e = await loadDay(key)
      if (e?.sources?.[id]?.records?.some((r) => FIELDS.some((f) => r[f] > 0))) {
        fromCache = key
        break
      }
    }
  ...
  const emit = (set, event) => {
    for (const listener of set) {
      try {
        listener(event)
      } catch {
        /* A closed consumer cannot fail a background batch. */
      }
    }
  }
  const enqueue = (fn) => {
  ...
  function initialize() {
    if (!initialized)
      initialized = (async () => {
        const m = await storage.read('metadata')
        if (m?.schemaVersion === SCHEMA && m.semantics === SEMANTICS) {
    ...
    return (
      ...
      SOURCES.every((id) => {
        ...
        return (
          ...
          Number.isFinite(Date.parse(s.cutoff)) &&
          Date.parse(s.cutoff) > midnight(key).getTime() &&
          Date.parse(s.cutoff) <= midnight(nextDay(key)).getTime() &&
          s.records.every(
            (r) =>
              typeof r.model === 'string' &&
              Number.isFinite(Date.parse(r.timestamp)) &&
              Date.parse(r.timestamp) >= midnight(key).getTime() &&
              Date.parse(r.timestamp) < Math.min(midnight(nextDay(key)).getTime(), Date.parse(s.cutoff)) &&
              FIELDS.every((f) => Number.isInteger(r[f]) && r[f] >= 0)
          )
        )
      })
    )
  ...
  async function discover() {
    ...
    discoveryPromise = (async () => {
      if (!earliestDate) {
        earliestDate = await (earliestFn || (() => findEarliestLogDate({ homeDir })))()
        const stored = await storage.list()
        const legacyDir = path.join(homeDir, '.ai-workbench', 'daily-stats')
        let archived = []
        if (!legacyReadFn) {
          try {
            archived = (await fs.readdir(legacyDir))
              .filter((n) => /^\d{4}-\d{2}-\d{2}\.json$/.test(n))
              .map((n) => n.slice(0, 10))
              .sort()
            for (const key of archived) {
              const old = await legacyRead(key)
              if (old?.summary?.total > 0) {
                stored.push(key)
                break
              }
            }
        ...
        if (archived.length) {
          const key = stored.find((k) => archived.includes(k))
  ...
  async function fillDay(key, cutoff, context, { force = false, retry = false } = {}) {
    ...
    for (const id of SOURCES) {
      ...
      try {
        ...
        const raw =
          exists === 'missing' ? [] : await scan(id, midnight(key), end, { windowContext: context, strictScan: true })
        if (!Array.isArray(raw)) throw Error('INVALID_SCAN_RESULT')
        const records = raw
          .filter(
            (r) =>
              new Date(r.timestamp).getTime() >= midnight(key).getTime() &&
              new Date(r.timestamp).getTime() < end.getTime()
          )
          .map((r) => ({
            timestamp: new Date(r.timestamp).toISOString(),
            model: typeof r.model === 'string' ? r.model : 'unknown',
            project: typeof r.project === 'string' ? r.project : '未知项目',
            ...Object.fromEntries(
              FIELDS.map((f) => [
                f,
                typeof r[f] === 'number' && Number.isFinite(r[f]) ? Math.max(0, Math.floor(r[f])) : 0,
              ])
            ),
          }))
        sources[id] = {
          status: exists === 'missing' ? 'missing' : 'ready',
          records,
          cutoff: end.toISOString(),
          complete: end.getTime() === midnight(nextDay(key)).getTime(),
        }
      } catch (error) {
    ...
    days.set(key, entry)
    if (end.getTime() < midnight(nextDay(key)).getTime()) openDays.add(key)
  ...
  function dates(start, end, cutoff) {
    if (!validDay(start) || !validDay(end) || end <= start || !Number.isFinite(cutoff.getTime()))
      throw Error('INVALID_RANGE')
    const keys = [],
      upper = Math.min(midnight(end).getTime(), cutoff.getTime())
  ...
  async function fillRange(start, end, options = {}) {
    await initialize()
    const cutoff = new Date(options.cutoff || dataCutoff || nowFn()),
      keys = dates(start, end, cutoff),
      context = createLogScanWindowContext(midnight(start))
    let changed = false
    for (const key of keys)
      changed =
        (await fillDay(key, cutoff, context, { force: options.force === true, retry: options.retry === true })) ||
        changed
    if (changed) {
      revision++
      if (!dataCutoff) dataCutoff = cutoff.toISOString()
      await storage.write('metadata', metadata())
      if (options.publish !== false) emit(listeners, { revision, cutoff: dataCutoff, today: dayKey(nowFn()) })
    }
    return keys
  }
  function ensureRange(start, end, options) {
    return enqueue(() => fillRange(start, end, options))
  }
  function calendarSnapshot(month, taskId) {
    const today = dayKey(nowFn()),
      end = nextDay(today),
      monthEnd = dayKey(new Date(Date.UTC(Number(month.slice(0, 4)), Number(month.slice(5)), 1) - 8 * 3600000))
    const keys = earliestDate
      ? dates(month + '-01', monthEnd, new Date(Math.min(midnight(end).getTime(), nowFn().getTime()))).filter(
          (k) => k >= earliestDate
        )
      : []
    const result = {
  ...
  async function getCalendar({ month, taskId, retryDate }, onProgress) {
    if (typeof month !== 'string' || !/^\d{4}-(0[1-9]|1[0-2])$/.test(month) || month > dayKey(nowFn()).slice(0, 7))
      throw Error('INVALID_MONTH')
    if (retryDate && (!validDay(retryDate) || retryDate.slice(0, 7) !== month || retryDate > dayKey(nowFn())))
    ...
    try {
      progress()
      if (retryDate)
        await ensureRange(retryDate, nextDay(retryDate), { cutoff: new Date(dataCutoff || nowFn()), retry: true })
      else if (earliestDate) {
        const start = month + '-01' > earliestDate ? month + '-01' : earliestDate
        const end = dayKey(new Date(Date.UTC(Number(month.slice(0, 4)), Number(month.slice(5)), 1) - 8 * 3600000))
        if (start < end) await ensureRange(start, end)
      }
      for (const key of Object.keys(calendarSnapshot(month, taskId).data.days)) demand.set(key, nowFn().getTime())
      progress()
      return calendarSnapshot(month, taskId).data
    } finally {
      progressListeners.delete(progress)
    }
  }
  ...
  async function getPlanTokens(id, cycle) {
    ...
    if (midnight(cycle.start) >= cutoff)
      return {
        records: [],
        pending: true,
        cutoff: cutoff.toISOString(),
        revision,
        metrics: { scans: 0, cacheHits: 0, records: 0, elapsedMs: 0 },
      }
    const before = scans,
      started = performance.now(),
      keys = await ensureRange(cycle.start, cycle.end, { cutoff }),
      records = []
    ...
    for (const key of keys) {
      ...
      records.push(
        ...s.records
          .filter((r) => Date.parse(r.timestamp) < Math.min(publishedCutoff.getTime(), midnight(cycle.end).getTime()))
          .map((r) => ({ ...r, source: id }))
      )
    }
    return {
      ...
      missingSource,
      pending,
      cutoff: publishedCutoff.toISOString(),
      revision,
      metrics: {
        scans: scans - before,
        cacheHits: scans === before ? keys.length : 0,
  ...
  async function getTodayAggregates() {
    await initialize()
    const today = dayKey(nowFn()),
      cutoff = new Date(dataCutoff || nowFn())
    ...
    return {
      models: aggregateByModel(records),
      projects: aggregateByProject(records),
  ...
  async function tick() {
    ...
    ticking = enqueue(async () => {
      ...
      const cutoff = nowFn(),
        today = dayKey(cutoff),
        open = [...openDays].filter((key) => key < today)
      for (const [key, touched] of demand) {
        if (cutoff.getTime() - touched > 600000) {
          demand.delete(key)
          continue
        }
        const e = await loadDay(key)
      ...
      lastRunAt = dataCutoff
      await storage.write('metadata', metadata())
      emit(listeners, { revision, cutoff: dataCutoff, today })
    }).finally(() => {
      ticking = null
    })
    return ticking
  ...
  async function bootstrap(cycles = []) {
    ...
    const today = dayKey(nowFn())
    if (earliestDate) {
      const start = today.slice(0, 7) + '-01' > earliestDate ? today.slice(0, 7) + '-01' : earliestDate
      await ensureRange(start, nextDay(today))
    }
  ...
  return {
    ...
    getPlanTokens,
    getTodayAggregates,
    getEarliestDate: discover,
    getSourceEarliestDate,
    readLegacyDay,
    tick,
    bootstrap,
    subscribe: (fn) => {
    ...
    snapshot: () => ({ revision, dataCutoff, lastRunAt, earliestDate, scans, queuedOpenDays: openDays.size }),
    async earliestLedgerDay() {
      await initialize()
      for (const key of (await storage.list()).sort()) {
        const e = await loadDay(key)
        if (e && (e.legacy?.summary?.total > 0 || recordsOf(e).some((r) => FIELDS.some((f) => r[f] > 0)))) return key
      }
      if (!legacyReadFn) {
        try {
          for (const name of (await fs.readdir(path.join(homeDir, '.ai-workbench', 'daily-stats'))).sort()) {
            const key = name.slice(0, 10)
            if (validDay(key) && (await legacyRead(key))?.summary?.total > 0) return key
          }
        } catch (e) {
          if (e.code !== 'ENOENT') throw e
        }
