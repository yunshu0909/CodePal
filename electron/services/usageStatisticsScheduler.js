...
function createUsageStatisticsScheduler({
  statistics,
  getCycles = async () => [],
  nowFn = () => new Date(),
  setTimer = setTimeout,
  ...
  return {
    async start() {
      ...
      busy = true
      try {
        const state = await statistics.bootstrap(await getCycles())
        const elapsed = state?.lastRunAt ? Math.max(0, nowFn().getTime() - Date.parse(state.lastRunAt)) : 0
        schedule(Math.max(1, 300000 - elapsed))
      } catch {
        onError()
        schedule()
      } finally {
