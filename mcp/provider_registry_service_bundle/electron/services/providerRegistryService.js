 ...
 * @returns {Array<{id: string, name: string, url: string, icon: string, color: string, supportsToken: boolean, source: string}>}
 */
function buildProviderCards(providerDefinitions) {
  ...
  return sortedEntries.map(([providerId, definition]) => ({
    ...
    url: definition.ui?.url || definition.defaultBaseUrl || '',
